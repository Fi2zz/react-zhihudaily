  ...
  if (typeof reducers === "object") {
    ...
    if (Array.isArray(reducers)) {
      ...
      for (let key in reducers) {
        localReducers[key] = typeof reducers[key] === "function" ? reducers[key](state) : reducers[key];
      }
    }
...
export class Store {
  ...
  createStore(state, reducers, middlewares, afterApplyMiddleware) {
    let store = createStore(combineReducer(state, reducers), applyMiddlewares(middlewares));
    if (typeof afterApplyMiddleware === "object" && !Array.isArray(afterApplyMiddleware)) {
