import { formatDateWithTime,padding } from "./utils";
import { createBrowserHistory } from "history";

export const history =createBrowserHistory();
const apiFactory = (name, query) => {
...
export function get(api, query) {
  let fetched = fetch(apiFactory(api, query), {
    method: "GET"
  });
  return fetched.then(res => {
    ...
    return Promise.reject({ code: res.status, reason: res.json(),status:res.status });
  });
}
